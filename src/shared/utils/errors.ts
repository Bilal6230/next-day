...
const SAFE_DOMAIN_ERROR_PATTERNS = [
  ...
  /^Phrase must be \d+ characters or less$/,
  /^Transliteration must be \d+ characters or less$/,
  /^Translation must be \d+ characters or less$/,
  /^Target count must be between \d+ and \d+$/,
  /^Each tag must be \d+ characters or less$/,
  /^Maximum \d+ tags allowed$/,
  /^Target count must be a whole number$/,
];
